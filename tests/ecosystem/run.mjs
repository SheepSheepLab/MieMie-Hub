...
for (let i=2;i<process.argv.length;i+=2) {
  ...
  if (!['--polisher','--metadata','--legacy-polisher'].includes(flag)||!value||flags.has(flag)) throw Error('Use --polisher <JSON> --metadata <update JSON> --legacy-polisher <published 1.0.1 JSON>.');
...
await mkdir(path.join(project,'test-results'),{recursive:true});await writeFile(path.join(project,'test-results/ecosystem.json'),JSON.stringify(report,null,2)+'\n');

async function runPolisherChecks(check) {
  ...
  await check('停用移除润色面板、入口与订阅，撤销 Hook，设置和备份不变',async()=>{
    ...
    await runtime().open('test.runtime');await menu();click('[data-hub-app="miemie.timeline"]');await until(()=>document.querySelectorAll('.ts-choice').length===2,'停用后时间线');
  ...
  await check('润色初始化部分失败能清理资源，通用测试夹具与时间线继续可用',async()=>{
    ...
    assert((await runtime().open('test.runtime')).ok,'通用测试夹具 被影响');await menu();click('[data-hub-app="miemie.timeline"]');await until(()=>document.querySelectorAll('.ts-choice').length===2,'错误后时间线');
  ...
  await check('润色脚本单独移除清理实例，重新载入保留原启用偏好',async()=>{
    ...
    assert(window.__timelineSwitcherV1&&(await runtime().open('test.runtime')).ok,'影响 Core');

...
第一版仅管理全局脚本。物理停用/启用通过正式 API 修改目标 `enabled`；禁用文件夹须先由用户启用。物理卸载会移除目标脚本条目和其 `data`，因此必须先导出恢复 JSON 并由用户确认实际保存。发起浏览器下载不代表文件已经永久保存。备份后脚本字段变化会中止删除。脚本之外的 localStorage、世界书和其他 Extension 数据不会清理。仅存在于 Runtime 的临时扩展仍只能“注销”，不能冒充物理卸载。
